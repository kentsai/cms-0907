...
export const COURSE_GROUP_LIST_FILTERS_KEY = 'course-group-list-filters';
export const COURSE_GROUP_LIST_SORT_KEY = 'course-group-list-sort';
export const COURSE_GROUP_LIST_PAGE_KEY = 'course-group-list-page';
...
@Component({
  ...
  styleUrl: './course-group-list.component.scss'
})
export class CourseGroupListComponent implements OnInit {
  private readonly service = inject(CourseGroupService);
  private readonly router = inject(Router);
  private readonly confirmation = inject(ConfirmationService);
  private readonly messages = inject(MessageService);

  protected readonly items = signal<CourseGroup[]>([]);
  ...
  ngOnInit(): void {
    this.filters = { ...EMPTY_COURSE_GROUP_QUERY, ...readSession<CourseGroupQuery>(COURSE_GROUP_LIST_FILTERS_KEY, {}) };

    const sort = readSession<SortState>(COURSE_GROUP_LIST_SORT_KEY, { sortField: 'pkid', sortOrder: -1 });
    this.sortField = sort.sortField;
    this.sortOrder = sort.sortOrder;

    const page = readSession<PageState>(COURSE_GROUP_LIST_PAGE_KEY, { first: 0, rows: 20 });
  ...
  protected search(): void {
    writeSession(COURSE_GROUP_LIST_FILTERS_KEY, this.filters);
    this.first = 0;
    writeSession(COURSE_GROUP_LIST_PAGE_KEY, { first: this.first, rows: this.rows });
  ...
  protected onSort(event: { field?: string; order?: number }): void {
    ...
    writeSession(COURSE_GROUP_LIST_SORT_KEY, { sortField: this.sortField, sortOrder: this.sortOrder });
  }
  ...
  protected view(item: CourseGroup): void {
    this.router.navigate(['/course/course-groups', item.pkid]);
  }

  protected edit(item: CourseGroup): void {
    this.router.navigate(['/course/course-groups', item.pkid, 'edit']);
  }

  protected confirmDelete(item: CourseGroup): void {
